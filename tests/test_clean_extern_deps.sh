...
mkdir -p "$TMP/scripts" "$TMP/extern/tenferro-rs" "$TMP/extern/pytorch-openblas" "$TMP/extern/keep-me"
cp "$ROOT/scripts/clean_extern_deps.sh" "$TMP/scripts/clean_extern_deps.sh"

touch "$TMP/extern/tenferro-rs/sentinel"
touch "$TMP/extern/pytorch-openblas/sentinel"
...
test ! -e "$TMP/extern/pytorch-openblas"
