...
class SmallWorkSchemaTests(unittest.TestCase):
    ...
    def test_reduce_sum_declares_owned_single_call_and_session_boundaries(self):
        suite = yaml.safe_load((ROOT / "benchmarks/cpu/small_work.yaml").read_text())
        cases = [case for case in validate_suite_contract(suite) if case.operation == "reduce_sum"]
        self.assertEqual(len(cases), 6)
        for size in (4, 16, 256):
            self.assertEqual({case.api_tier for case in cases if case.shape == (size,)}, {"concrete-fresh", "concrete-shared"})
        for case in cases:
            self.assertEqual(case.contract_id, "core.reduce_sum.ordinary.concrete")
            self.assertEqual(case.dtype, "f64")
            self.assertEqual(case.calls_per_workflow, 1)
            self.assertIn("reduce_sum", case.scope_timer)
